...
/** Format bid/ask respecting per-pair rounding decimals */
function fmtBidAsk(value: number, roundingDecimals: number | null): string {
  if (!value) return '—';
...
// ─── Spread save callback type ──────────────────────────────

interface SpreadChangePayload {
...
const ComparisonRow = memo(function ComparisonRow({
  ...
  onSpreadChange: (quote: string, spreadType: SpreadType, spreadMode: SpreadMode, fixedUnit: FixedUnit, values: SpreadChangePayload) => void;
}) {
  ...
  const localRoundingDecimals = spreadDirty && editRoundingDecimals != null
    ? editRoundingDecimals
    : (row.storeRate.roundingDecimals != null ? String(row.storeRate.roundingDecimals) : '');

  const saveSpread = () => {
    const roundingDec = localRoundingDecimals !== '' ? parseInt(localRoundingDecimals, 10) : null;
  ...
  return (
    ...
    >
      ...
      <td className="px-3 py-3 text-right font-mono text-[12px] text-text-primary">
        {fmtBidAsk(row.storeRate.bid, row.storeRate.roundingDecimals)}
      </td>

      {/* Ask */}
      <td className="px-3 py-3 text-right font-mono text-[12px] text-text-primary">
        {fmtBidAsk(row.storeRate.ask, row.storeRate.roundingDecimals)}
      </td>
      ...
      <td className="px-3 py-3">
        <div className="flex items-center gap-1.5">
          <input
            ...
            onChange={(e) => { setEditRoundingDecimals(e.target.value); setSpreadDirty(true); }}
            onKeyDown={(e) => { if (e.key === 'Enter') saveSpread(); }}
  ...
  const handleSpreadChange = async (
    ...
    try {
      await spreadApi.update({
        ...
        spreadMode,
        fixedUnit,
        spreadPercent: values.spreadPercent,
        spreadFixed: values.spreadFixed,
        buyMargin: values.buyMargin,
        sellMargin: values.sellMargin,
        roundingDecimals: values.roundingDecimals,
      ...
      <div className="card relative">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-terminal-border">
                ...
                <th className="table-header text-left px-3 py-3 min-w-[200px]">Spread</th>
                <th className="table-header text-right px-3 py-3 min-w-[90px]">Bid</th>
                <th className="table-header text-right px-3 py-3 min-w-[90px]">Ask</th>
        ...
        <div className="px-5 py-3 border-t border-terminal-border flex items-center gap-2 text-xs text-text-muted">
          ...
          <span>
            ...
            <strong className="text-text-primary">Fixed</strong>: offset by fixed amount.{' '}
            <strong className="text-text-primary">Asymmetric</strong>: separate buy/sell margins.{' '}
            Customer receives the Bid rate when selling {baseCurrency}.
          </span>
        </div>
      </div>
