...
export type LevelPointsType = 'PERCENTAGE' | 'FIXED';

export interface LevelPointEntry {
  level: number;
  points: number;
}

export interface LevelPointsResponse {
  base: string;
  quote: string;
  pointsType: LevelPointsType;
  fixedUnit: FixedUnit;
  levels: LevelPointEntry[];
}

export const levelPointsApi = {
  get: (base: string, quote: string) =>
    get<LevelPointsResponse>('/rates/level-points', { base, quote }),
  update: (dto: { base: string; quote: string; pointsType?: LevelPointsType; fixedUnit?: FixedUnit; levels: LevelPointEntry[] }) =>
    post<LevelPointsResponse>('/rates/level-points', dto),
};
