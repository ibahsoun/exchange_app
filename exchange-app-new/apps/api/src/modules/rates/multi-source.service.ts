...
export class MultiSourceService {
  ...
  async getAllSpreads(baseCurrency = 'USD') {
    ...
    return storeRates.map((sr) => {
      ...
      const { bid, ask, spread } = computeBidAsk(convertedMid, config, rounding);
  ...
  private convertBoardToBase(
    ...
    for (const row of board.rows) {
      ...
      if (row.quote === baseCurrency) {
        ...
        const { bid, ask, spread } = computeBidAsk(newMid, config, rounding);
  ...
  private resolveStoreRate(
    ...
    if (!existing) {
      const { bid, ask, spread } = computeBidAsk(avgMid || 0, config);
      return {
    ...
    const { bid, ask, spread } = computeBidAsk(mid, config, rounding);
  ...
  async getLevelPoints(base: string, quote: string) {
    const rows = await this.prisma.customerLevelPoint.findMany({
      where: { base, quote },
      orderBy: { level: 'asc' },
    });
    // Determine pointsType and fixedUnit from existing rows (all levels share the same type per pair)
    const pointsType = rows.length > 0 ? (rows[0].pointsType as string) : 'PERCENTAGE';
    const fixedUnit = 'RAW';
    // Always return all 5 levels, filling missing ones with 0
    const result: { level: number; points: number }[] = [];
    for (let lvl = 1; lvl <= 5; lvl++) {
      const row = rows.find((r) => r.level === lvl);
      result.push({ level: lvl, points: row ? Number(row.points) : 0 });
    }
    return { base, quote, pointsType, fixedUnit, levels: result };
  }

  async updateLevelPoints(dto: {
    base: string;
    quote: string;
    pointsType?: string;
    fixedUnit?: string;
    levels: { level: number; points: number }[];
  }) {
    const pointsType = dto.pointsType === 'FIXED' ? 'FIXED' : 'PERCENTAGE';
    const fixedUnit = 'RAW';
    const ops = dto.levels
      .filter((l) => l.level >= 1 && l.level <= 5)
      .map((l) =>
        this.prisma.customerLevelPoint.upsert({
          where: {
            base_quote_level: { base: dto.base, quote: dto.quote, level: l.level },
          },
          create: {
            base: dto.base,
            quote: dto.quote,
          ...
          update: { points: l.points, pointsType, fixedUnit },
        }),
      );
    await Promise.all(ops);
    return this.getLevelPoints(dto.base, dto.quote);
  }

  async getLevelPointsForCustomer(base: string, quote: string, level: number): Promise<{ points: number; pointsType: string; fixedUnit: string }> {
    const row = await this.prisma.customerLevelPoint.findUnique({
      where: { base_quote_level: { base, quote, level } },
    });
    return {
      points: row ? Number(row.points) : 0,
      pointsType: row?.pointsType ?? 'PERCENTAGE',
      fixedUnit: 'RAW',
    };
  }
